...
ensure_ghcr_login() {
    ...
    command -v gh >/dev/null 2>&1 || {
        echo "gh is required to authenticate to the private Core image" >&2
        return 1
    }
...
AI_CORE_IMAGE="${AI_CORE_IMAGE:-ghcr.io/safrano9999/fedora44-ai-core:latest}"
PULL_POLICY=always
[[ "$AI_CORE_IMAGE" != localhost/* ]] || PULL_POLICY=missing
[[ "$AI_CORE_IMAGE" != ghcr.io/* ]] || ensure_ghcr_login

BUILD_ARGS=(--build-arg "AI_CORE_IMAGE=$AI_CORE_IMAGE")
...
IMAGE="${FEDORA44_AI_BASE_IMAGE:-localhost/fedora44-ai-base:latest}"
