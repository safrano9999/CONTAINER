...
render_for_image() {
    local image="$1"
    local compose="$SCRIPT_DIR/$COMPOSE_FILE"
    local quadlet="$SCRIPT_DIR/$QUADLET_FILE"
    local temporary
...
if [ -z "$IMG_CHOICE" ]; then
    ...
    echo "    (2) Build locally from the core context [$LOCAL_IMAGE]"
...
case "$IMG_CHOICE" in
    1)
        ...
        podman pull --retry 10 --retry-delay 5s "$CONFIGURED_IMAGE"
        render_for_image "$CONFIGURED_IMAGE"
        ;;
    2)
        build_args=()
