...
BASE_RUNTIME_REPO="${FEDORA44_AI_BASE_REPO:-fedora44-ai-base}"
...
case "$FEDORA44_AI_VARIANT" in
    ...
    build) REPOS=("${BASE_REPOS[@]}" "${SAFRANO_REPOS[@]}") ;;
    safrano9999) REPOS=("$BASE_RUNTIME_REPO" "${BASE_REPOS[@]}" "${SAFRANO_REPOS[@]}") ;;
esac

distill_repo_examples() (
    local repo="${1%@*}"
    shift
    local repo_dir="$SAFRANO_DIR/$repo"
    local plugin_link="$repo_dir/safrano9999"

    [ -f "$SAFRANO_SCRIPTS_DIR/merge.sh" ] || { echo "Missing merge.sh" >&2; exit 1; }
    if [ -L "$plugin_link" ] && [ "$(readlink -f "$plugin_link")" = "$(readlink -f "$SAFRANO_DIR")" ]; then
        rm -f -- "$plugin_link"
    elif [ -e "$plugin_link" ] || [ -L "$plugin_link" ]; then
        echo "Cannot distill $repo: $plugin_link already exists" >&2
        exit 1
    fi
    trap 'rm -f -- "$plugin_link"' EXIT HUP INT TERM
    ln -s "$SAFRANO_DIR" "$plugin_link"
    (cd "$repo_dir" && bash "$SAFRANO_SCRIPTS_DIR/merge.sh" "$@")
)
...
case "$FEDORA44_AI_VARIANT" in
    ...
    safrano9999)
        distill_repo_examples "$BASE_RUNTIME_REPO" "${BASE_REPOS[@]}"
        bash "$SCRIPT_DIR/merge.sh" "$BASE_RUNTIME_REPO" "${SAFRANO_REPOS[@]}"
        ;;
esac
